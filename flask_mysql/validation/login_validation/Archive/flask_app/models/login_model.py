...
class User:
    ...
    def add_user(cls, data):
        query = "INSERT INTO login_users (first_name, last_name, email, password, created_at, updated_at) VALUES (%(first_name)s, %(last_name)s, %(email)s, %(password)s, NOW(), NOW() )"
        connectToMySQL('login').query_db(query, data)

    @classmethod
    def get_user(cls, data):
        query = "SELECT * FROM login_users WHERE email = %(email)s"
    ...
    def validate_user(data):
        ...
        if len(data['email']) < 4 or len(data['email']) > 255:
            is_valid = False
            flash('Email is too short or too long.')
